...
def InertiaFromTrajectory(t_xyz_xyzw, body_to_imu_xyz_xyzw, gravity):

    body_angle_rate_xyz = AngleRateInBody(t_xyz_xyzw[0], t_xyz_xyzw[4:])
    body_angle_rate_txyz = np.vstack((t_xyz_xyzw[0], body_angle_rate_xyz))
    imu_angle_rate_xyz = StaticTransformVec3d(body_to_imu_xyz_xyzw, body_angle_rate_xyz)
    imu_angle_rate_txyz = np.vstack((t_xyz_xyzw[0], imu_angle_rate_xyz))
    
    temporal_rot = np.array([[0,1,0],[0,0,1],[1,0,0]])
    imu_angle_rate_txyz[1:] = np.array([temporal_rot.dot(imu_angle_rate_txyz[1:].transpose()[i]) for i in range(len(t_xyz_xyzw[0]))]).transpose()
    imu_acceleration = AccelInIMU(t_xyz_xyzw[0], t_xyz_xyzw[1:4], t_xyz_xyzw[4:], gravity, body_to_imu_xyz_xyzw)
