...
class BsplineFittingProblem(Problem):
    def __init__(self, degree, knot_vec, pts, pys):
        self.bsp = bspline(degree, knot_vec, [1.]*(len(knot_vec) - degree - 1))#copy.deepcopy(bsp)
        self.pts = pts[1:-1]
        self.pys = pys[1:-1]
...
if __name__ == "__main__":
    ...
    knot_vec = CreateUniformKnotVector(3, 1, 16, 14)
    # print(knot_vec)
    # bsp = bspline(3, knot_vec, [1.]*(len(knot_vec) - 4))
    problem = BsplineFittingProblem(3, knot_vec, sample_data[0], sample_data[1])
    ...
    # quit()
    result = problem.solve(guess, step=10)
